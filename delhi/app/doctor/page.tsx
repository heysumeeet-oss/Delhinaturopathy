...
export const metadata: Metadata = {
  title: `${DOCTOR.name} - Best Naturopath in Delhi | Profile`,
  description: `Meet ${DOCTOR.name}, Senior Naturopath in Delhi with ${DOCTOR.experience} experience. Expert in Chronic Pain, Diabetes Reversal & Detoxification.`,
  alternates: {
    canonical: '/doctor',
  },
};
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-grow pt-20">
        ...
        {/* Profile Header */}
        <div className="bg-white border-b border-light-gray/70">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20">
            <div className="flex flex-col md:flex-row items-center gap-12">

                {/* Image */}
                <div className="relative flex-shrink-0">
                <div className="w-64 h-64 md:w-80 md:h-80 relative rounded-full overflow-hidden border-4 border-primary-blue shadow-2xl">
                    <Image
                    src={DOCTOR.image}
                    alt={DOCTOR.name}
                    fill
                ...
                <div className="absolute bottom-4 right-4 bg-primary-blue text-white px-4 py-1 rounded-full text-sm font-bold shadow-lg">
                ...
                <span className="text-primary-blue font-bold tracking-widest uppercase text-xs mb-2 block">Our Lead Specialist</span>
                <h1 className="text-4xl md:text-5xl font-serif font-bold text-dark-gray mb-4">{DOCTOR.name}</h1>
                <p className="text-xl text-dark-gray/70 font-medium mb-6">{DOCTOR.title}</p>

                <div className="flex flex-wrap justify-center md:justify-start gap-3 mb-8">
                    {DOCTOR.specializations.map((spec, idx) => (
                    <span key={idx} className="bg-primary-light text-primary-blue px-3 py-1 rounded-lg text-sm font-medium border border-primary-blue/20">
                        {spec}
                    </span>
                    ))}
                </div>
