    ...
    <section className="relative h-[100vh] flex content-center items-center justify-center text-white overflow-hidden">
      ...
      <div className="absolute top-0 w-full h-full z-0">
        <Image
          ...
          alt="Serene Yoga and Meditation Space"
          fill
          priority
          className="object-cover object-center"
        />
        <div className="absolute inset-0 bg-gradient-to-br from-black/60 via-primary-blue/40 to-black/60 mix-blend-multiply z-10"></div>
        <div className="absolute inset-0 bg-gradient-to-t from-primary-blue/90 via-transparent to-transparent opacity-80 z-10"></div>
      </div>
      ...
      <div className="container relative mx-auto px-4 z-20 flex flex-col items-center justify-center text-center">
        <div className="w-full lg:w-9/12 xl:w-8/12 animate-fade-in-up">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-6 animate-slide-up opacity-0" style={{ animationDelay: '0.8s' }}>
            ...
            >
              <span className="relative z-10 flex items-center group-hover:text-primary-dark transition-colors">
                Schedule Consultation
                <ArrowRight className="ml-2 w-4 h-4 group-hover:translate-x-1 transition-transform" />
              </span>
              <div className="absolute inset-0 bg-primary-light transform scale-x-0 group-hover:scale-x-100 transition-transform origin-left duration-300"></div>
            </Link>
            ...
            >
               <span className="mr-2 w-6 h-6 rounded-full bg-white text-accent-teal flex items-center justify-center group-hover:scale-110 transition-transform">
              ...
              { icon: null, label: "Acupressure", customIcon: "P" }
               ...
               <React.Fragment key={index}>
                  <div className="flex items-center gap-3 group cursor-default">
                    {item.icon ? (
                       <item.icon className="w-5 h-5 text-primary-light/70 group-hover:text-white transition-colors" />
                    ) : (
                       <span className="w-5 h-5 flex items-center justify-center border border-primary-light/70 rounded-full text-[10px] text-primary-light/70 group-hover:text-white group-hover:border-white transition-colors">{item.customIcon}</span>
                    )}
